...
% varargin   command line arguments to Sync_station_v3 (see VARARGIN)

% Choose default command line output for Sync_station_v3
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Sync_station_v3 wait for user response (see UIRESUME)
...
handles.Cfact=samplingF/framerate;
    ...
    buffer=str2double(get(handles.edit3_buffer,'String'));
    
    time_sec=round(handles.frame*handles.Cfact)-delay+buffer;
...
[handles.videofilename, handles.pathname]=uigetfile({'*.MOV;*.avi;*.MP4;*.mp4;*.seq','Video files';'*.tif;*.jpg;*.bmp', 'Image files'},'pick file');
...
vid_dur_sec=handles.totalframes*(1/handles.framerate);
vid_dur_accel_frames = vid_dur_sec * str2double(get(handles.edit_accelrate,'String'));

end_vid=round(start+vid_dur_accel_frames);
...
while(handles.frame+step<handles.totalframes)
...
if get(handles.radiobutton1_zoom, 'Value')==0
    ...
    time_sec=round((handles.frame*handles.Cfact)+buffer);
